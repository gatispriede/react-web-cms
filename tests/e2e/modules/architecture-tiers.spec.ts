...
const scenario = buildScenario('architecture-tiers');

test.describe.serial('module — ArchitectureTiers admin + client', () => {
    test('admin authors an ArchitectureTiers title on a fresh page', async ({adminPage}) => {
        await createPage(adminPage, scenario);
        await addBlankSection(adminPage);
        await openAddModuleDrawer(adminPage, EItemType.ArchitectureTiers);
        await fillPrimaryText(adminPage, scenario.marker);
        await saveModuleDrawer(adminPage);
    });

    test('public site renders the ArchitectureTiers marker', async ({anonPage}) => {
