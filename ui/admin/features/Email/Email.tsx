/**
 * admin-module-composed — Email config bridge.
 *
 * Was a bespoke hand-coded pane; now the `AdminLoader` *bridge* for
 * `system/email`. `EmailViewModel` is unchanged ("admin stays mostly
 * same") — only the surrounding Card chrome moves into the generic
 * `AdminFormModule` shape. The provider config `<Form>` becomes the
 * module's `children`; Save maps onto the module's `onSave`. The
 * intro copy + the separate "Test send" Card stay bespoke in the body.
 *
 * Registered with the `AdminPageRegistry` by `EmailAdminLoader`; the
 * shell reaches it via `AdminPageDispatch` (see `EmailAdminUILoader`).
 */
