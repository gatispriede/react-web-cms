 ...
 *
 * admin-module-composed — Currency settings bridge.
 *
 * Was a bespoke hand-coded pane; now the `AdminLoader` *bridge* for
 * `system/currency`. `CurrencySettingsViewModel` is unchanged ("admin
 * stays mostly same") — only the surrounding chrome moves into the
 * generic `AdminFormModule` shape. The provider `<Form>` + Stripe Tax
 * Card become the module's `children`; Save maps onto `onSave`; the
 * surface error maps onto the module's `error`.
 *
 * Registered with the `AdminPageRegistry` by `SiteAdminLoader`.
 */
import React, {useEffect} from 'react';
import {Badge, Card, Form, Select, Space, Switch, Typography} from 'antd';
...
const CurrencySettings: React.FC = () => {
    ...
    return (
        <AdminFormModule
            testId="currency-settings-pane"
            title={t('pricing.section.currency', {defaultValue: 'Currencies'})}
            error={vm.err}
            onSave={() => void vm.save()}
            saveLabel={t('actions.save', {defaultValue: 'Save'})}
            saveTestId="currency-save"
            saving={vm.saving}
        >
