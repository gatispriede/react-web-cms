'use client';
/**
 * Client view for `/auth/signin` — App Router migration, Batch 6.
 * Direct lift of the visible chrome from `pages/auth/signin.tsx`.
 *
 * `useTranslation` swapped from `next-i18next/pages` to the v16 client
 * subpath `next-i18next/client` — App-Router-native, no `appWithTranslation`
 * HOC needed (the root layout's `<I18nProvider>` already hydrated).
 */
...
const SignInLegacyView: React.FC<{callbackUrl: string}> = ({callbackUrl}) => {
    const {t} = useT('common');
    ...
    // Drive a 1s tick only while a lockout is active. We stop the interval
    // as soon as the wait elapses so we're not burning timers when idle.
    useEffect(() => {
        if (lockedUntil <= now) {
    ...
    const onFinish = async (values: {email: string; password: string}) => {
        ...
        try {
            ...
            if (res?.error) {
                ...
                setErrorText(message || (t('Sign in failed') as string));
                if (retryMs > 0) {
            ...
            setErrorText(e?.message || (t('Sign in failed') as string));
        } finally {
            setSubmitting(false);
        }
    };
    ...
    return (
        <div style={{minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16, background: '#f5f5f5'}}>
            <div style={{width: '100%', maxWidth: 380, background: '#fff', padding: 24, borderRadius: 8, boxShadow: '0 2px 8px rgba(0,0,0,0.08)'}}>
                <h2 style={{marginTop: 0, marginBottom: 16}}>{t('Sign in') as string}</h2>
                {/* Reason (e.g. "Wrong email or password") is always shown so
                    the user understands WHY they're locked, not just that they
                    are. The countdown banner sits below it and ticks live. */}
                {errorText ? (
...
export default SignInLegacyView;
