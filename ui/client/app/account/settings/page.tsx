/**
 * `/account/settings` — module-compose pass 2026-05-17.
 *
 * Keeps the existing SSR data pipeline (customer profile + site flags
 * resolved via `getCustomerProfileService` + `SiteFlagsService`) but
 * delivers the result to the locked `AccountSettingsLayout` module
 * via the SystemPageDispatch `pageProps` channel introduced in the
 * same commit. No client refetch flash; the locked module reads the
 * server-resolved shape directly from props.
 */
...
export default async function AccountSettingsPage({
    ...
    const systemPage = loadSystemPageSnapshot('account-settings');
    ...
    if (!conn.database) {
        ...
        return <AccountSettingsView systemPage={systemPage} me={me} activeTab={activeTab} hiddenTabs={[]} enabled={false}/>;
    }
