'use client';
/**
 * Client view for `/account/verify` — App Router migration, Batch 6.
 * Direct lift of the visible chrome from `pages/account/verify.tsx`.
 *
 * W6c — magic-link verify (click-to-confirm) page. Defeats cross-device
 * pre-fetch: email clients on mobile (Outlook, Apple Mail) frequently
 * fetch every link in the inbox preview pipeline with a HEAD/GET —
 * that would burn a single-use token before the human ever clicks. We
 * render a confirmation button on GET and only redeem the token on an
 * explicit POST (the user-initiated click).
 */
...
const VerifyView: React.FC<{token: string | null; callbackUrl: string}> = ({token, callbackUrl}) => {
    ...
    const onConfirm = async () => {
        ...
        try {
            ...
            if (res?.ok) {
                ...
                // cookie to `attachMarketingSession`. Best-effort: a
                // telemetry miss never blocks the redirect.
                try {
