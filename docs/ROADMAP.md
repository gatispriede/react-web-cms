...
- [First-class themes — Stitch-designed, full coverage](roadmap/storefront/first-class-themes.md) — replace 4 token-reskin presets with ≥5 Stitch-designed themes; each ships design doc + frames covering every module type. **XL** (multi-week, parallelisable per theme). 2/5 shipped 2026-05-13: **Editorial** (warm cream / Source Serif Pro / gentle-slow) + **Commerce** (pure white / Space Grotesk / emerald / snappy-default — home theme for the cars vertical). Per-theme baseline specs at `tests/e2e/visual/themes/{editorial,commerce}.spec.ts` (110 tests total await operator capture run). Remaining 3 themes (SaaS-landing, Restaurant, Event/Agency) follow the same template.
