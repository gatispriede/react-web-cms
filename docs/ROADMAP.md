...
- [First-class themes — Stitch-designed, full coverage](roadmap/storefront/first-class-themes.md) — replace 4 token-reskin presets with ≥5 Stitch-designed themes; each ships design doc + frames covering every module type. **XL** (multi-week, parallelisable per theme). 1/5 shipped 2026-05-13: **Editorial** (warm cream + ink + terracotta palette, Source Serif Pro display + Inter body, gentle-slow motion, full-bleed overlay-nav header, hairline radii, 47-module SCSS pass, per-theme baseline spec at `tests/e2e/visual/themes/editorial.spec.ts` — 55 tests await operator capture run). Remaining 4 themes (Commerce, SaaS-landing, Restaurant, Event/Agency) follow the same template.
