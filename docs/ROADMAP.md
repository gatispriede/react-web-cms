...
- [Module-compose every customer page](roadmap/platform/all-pages-module-composed.md) — Phase 1.C/1.D + Account/Auth/Blog/Cars batches shipped: products + checkout + account dashboard + magic-link + orders + addresses + inbox + signin + signup + blog + cars are all module-composed. **2026-05-17 pilot**: `/account` home now module-composed via new `EItemType.AccountDashboardGrid` + `AccountDashboardGridHost` smart wrapper + `account-home` system-page registration. Profile + settings + signin/verify pages remain as the last bespoke customer surfaces; same pattern applies, ~30 min per page.
