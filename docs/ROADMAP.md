...
- **F3 — `/v1/**` namespace migration** — in flight 2026-05-03 (paused on `git mv` permission). Decisions resolved: prefix `/v1/`, redirect window 1 release cycle, `public/` untouched, sharp break is OK. Scope: 30 files under `pages/admin/` + 29 under `pages/api/` move to `pages/v1/`; ~200 internal callsites swept (`fetch('/api/')`, `<Link href="/admin/">`); existing `next.config.js` `redirects()` extends with 308 v1 rules; existing `rewrites()` retargets `/v1/api/robots.txt` and `/v1/api/sitemap.xml`; locale-prefix admin redirects retarget `/v1/admin`. **Open: should the `services/` source directory mirror the move (`@services/` → `@services/v1/`)?** Rationale per user: per-route version splits enable mixed-version deployments. See [roadmap/v1-url-namespace.md](roadmap/v1-url-namespace.md).
