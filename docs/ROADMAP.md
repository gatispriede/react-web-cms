...
- ~~**Admin-IA nav rail regression (bug)**~~ — ✅ shipped 2026-05-17. Parent-bucket override map (`PARENT_BUCKET_OVERRIDES`) + `resolveActiveArea()` helper in `adminAreaItems.ts`. Cross-area links in the 5-bucket rails (Content → Publishing, Settings → Theme, Analytics → Audit log, System → Bundle, etc.) now keep the parent bucket's rail visible + the top-bar button highlighted. 13 unit tests pin the override map + fallback semantics.
- ~~[Module-compose every customer page](roadmap/platform/all-pages-module-composed.md)~~ — ✅ shipped 2026-05-17. The Account batch closed: `/account` home + `/account/profile` + `/account/verify` + `/account/settings` all module-composed via new EItemTypes (`AccountDashboardGrid`, `AccountProfileForm`, `CustomerVerifyConfirm`, `AccountSettingsLayout`). `/account/settings` exercised the new **SystemPageDispatch `pageProps` channel** so SSR-resolved `me + hiddenTabs + enabled` reach the locked module without a client refetch flash. 4 unit tests pin the pageProps contract; new e2e spec at `tests/e2e/account/module-composed-pages.spec.ts` covers all four pages. Every customer-facing URL now renders through a system-page registration + module dispatch.
...
- **F8 MCP follow-ups** — streaming transport for bundle/image ops (✅ partly shipped 6c091be), ~~plugin SDK for third-party tools~~ ✅ shipped 2026-05-17 (`services/features/Mcp/plugin/` — `registerMcpToolPlugin(plugin)` with namespace prefixing + audit-scope inheritance + onRegister lifecycle hook; `buildToolRegistry()` merges plugin tools alongside built-ins; 11 unit tests). E2E un-skip post Windows-fanout fix remains. See [roadmap/platform/mcp-real-world-ready.md](roadmap/platform/mcp-real-world-ready.md).
...
- ~~[customer-notification-preferences.md](roadmap/storefront/customer-notification-preferences.md)~~ — ✅ shipped. `services/features/Notifications/` carries the preference matrix + quiet-hours queue + digest worker + unsubscribe tokens + email-suppression; `NotificationInbox` module with unread count + module-composed `/account/inbox` system page (via `CustomerAccountPages.ts`); preference center under `/account/settings?tab=notifications` via `AccountSettingsLayout`; 4 MCP tools (`notification.preferences.get/set` + `notification.stats` + `notification.inbox.list`). Roadmap entry was simply stale.
- ~~[multi-currency-and-tax.md](roadmap/storefront/multi-currency-and-tax.md)~~ — ✅ shipped (commit `edea545`).
- ~~[seo-program.md](roadmap/storefront/seo-program.md)~~ — ✅ shipped. `/sitemap.xml` (sitemap index) + per-feature sub-sitemaps via `sitemapContributors.ts`; `/robots.txt` env-gated + AI-bot opt-out (GPTBot / ClaudeBot / Google-Extended); `/api/og/<slug...>` PNG via `@vercel/og` (Twitter-card bugfix landed 2026-05-17); `RedirectsService` + `/admin/system/redirects` operator pane; `SchemaOrgInjector` JSON-LD primitive; `SeoHead` canonical + hreflang + OG/Twitter card emission; MCP `seo.*` tools. Open operator decisions (sitemap-refresh strategy, GSC integration) deferred — recommended defaults shipped.
...
- **8 first-class themes shipped** — editorial, commerce, agency, saas-landing, restaurant, portfolio, local-business + Paper (v5). Each carries per-theme baseline specs. Radical per-module structural variants for Gallery / Hero / Timeline + (2026-05-17) ProjectGrid / OrderProgressTimeline / Footer / Services / FeatureGrid across all themes. Per-theme **kinetic-typography signatures** (CSS `animation-timeline: view()`) on hero headlines — editorial = letter-space pulse, agency = horizontal slide, commerce = quiet rise, saas-landing = rise + scale-out, restaurant = soft letter-space, portfolio = parallax drift, local-business = slab rise. Per-theme **motion signatures** override the shared `.reveal-on-scroll` with a theme-distinct entrance flavor (editorial slide-left, agency cinematic slide-left, commerce quiet rise, saas-landing zoom-in, restaurant tilt-in, portfolio drift, local-business poster tilt) plus per-theme **hover signatures** on cards (sharp slab for agency, soft scale for portfolio, elevation+shadow for saas, warm rotate for restaurant, slab-with-warm-shadow for local-business). Every theme now carries a recognizable feeling in animation, view, and visual — not just color.
