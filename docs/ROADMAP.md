...
- **8 first-class themes shipped** — editorial, commerce, agency, saas-landing, restaurant, portfolio, local-business + Paper (v5). Each carries per-theme baseline specs. Radical per-module structural variants for Gallery / Hero / Timeline + (2026-05-17) ProjectGrid / OrderProgressTimeline / Footer across all themes. Per-theme kinetic-typography signatures (CSS `animation-timeline: view()`) — editorial = letter-space pulse, agency = horizontal slide, commerce = quiet rise, saas-landing = rise + scale-out, restaurant = soft letter-space, portfolio = parallax drift, local-business = slab rise. Tracks the 2026 type-as-hero / kinetic-motion design trend (Awwwards / Figma / Creative Bloq).
