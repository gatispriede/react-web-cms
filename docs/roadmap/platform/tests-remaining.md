...
- [`ui/client/api/MongoApi.test.ts`](../ui/client/api/MongoApi.test.ts) — new. 6 tests mock each `*Api` constructor and assert the facade forwards every public method with the original args. Catches typos in delegate names, missed args, accidental transformation. Stops the facade from silently dropping methods during refactors.
- [`ui/client/lib/conflict.test.ts`](../ui/client/lib/conflict.test.ts) — new. 9 tests lock the `parseMutationResponse` / `isConflictPayload` / `ConflictError` contract that the rolled-out conflict-aware editors depend on.
- [`ui/client/features/Themes/googleFonts.test.ts`](../ui/client/features/Themes/googleFonts.test.ts) — new. 10 tests cover `extractFontFamily` / `buildFontStack` / `buildGoogleFontsUrl` and the catalogue shape so a hand-edit of `google-fonts.json` or a regression in `_document.tsx` font composition fails the suite.
...
1. **`LoginBtn` test** (`ui/client/features/Auth/login-btn.test.tsx`) — mock `next-auth/react` at the module level (`vi.mock('next-auth/react')`). Test three states: `{ status: 'loading' }` (renders loading indicator), `{ status: 'unauthenticated' }` (renders login button), `{ status: 'authenticated', data: { user: {...} } }` (renders user name + logout button). ~1–2 h.
...
Infra already in place — see `ui/client/lib/useAutosave.test.tsx` for the pattern. Use `// @vitest-environment jsdom` directive per file.
...
- `ui/client/features/Auth/login-btn.test.tsx` (new)
- `ui/client/modules/*.test.tsx` (new, per type)
- `ui/client/api/MongoApi.test.ts` (new)
- `ui/client/pages/api/*.integration.test.ts` (new)
