...
|------|-------|------|-------|
| [first-class-themes.md](storefront/first-class-themes.md) | 5 | XL (multi-week) | 4 of 8 themes still need design pass — `local-business`, `portfolio`, `agency`, `commerce`. `editorial` / `restaurant` / `event` / `saas-landing` shipped. Each remaining theme needs Stitch frames + per-module SCSS pass. |
...
| Per-theme Stitch design pass × 4 remaining themes | first-class-themes |
