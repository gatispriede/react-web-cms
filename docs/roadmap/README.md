...
| [invoicing-and-bookkeeping.md](storefront/invoicing-and-bookkeeping.md) | 7 | L (1-3 days AI) | NEW 2026-05-16. Every paid order produces a regulator-grade `IInvoice` (sequential number, full operator + customer details, line-level VAT breakdown, regime tag, wholesale-cost snapshot for COGS / margin tracking). Credit notes for refunds. PDF generation. Customer self-service download. Admin pane + MCP + bookkeeping CSV export. Builds on W8g VAT regime resolution + W6a receipt emails + dropship `quoteWholesale` snapshot. Pre-public-deploy adjacent — EU compliance needs this. |
