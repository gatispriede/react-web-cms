...
| `next-i18next/pages` → `/client` sweep | 2 | S (~1-2h AI) | Surfaced from B7 cutover. 11 component files still import from `next-i18next/pages` (admin tree + cars modules + a few client files). Works under `next-i18next@16` which exposes both subpaths concurrently, but blocks dropping the pages-router peer-dep and the eventual `next-auth@5` jump. Mechanical sweep onto `next-i18next/client`. |
...
| Item | Track | Size | Notes |
|------|-------|------|-------|
| ~~[admin-information-architecture.md](admin/admin-information-architecture.md)~~ | ~~admin~~ | ~~L-XL~~ | **SHIPPED 2026-05-16 (hybrid)** — new 6-bucket taxonomy (Site / Content / Commerce / People / Analytics / System) wired into the top bar + area rails + AdminView union; full pane audit at `docs/roadmap/_meta/admin-pane-inventory.md`; 6 demonstrator panes' URLs moved with 301 shim entries; shared `<PaneHeader>` / `<EmptyState>` / `<SaveBar>` + 4 rhythm tokens adopted in 6 demonstrator panes (one per bucket). Remaining ~40 panes' chrome swap + their per-loader URL moves captured as per-area sweep follow-ups below. See [shipped.md](shipped.md#2026-05-16--admin-information-architecture-new-taxonomy--shared-pane-components). |
| admin-ia per-area sweeps (Site / Content / Commerce / People / Analytics / System) | admin | M-L per bucket (6 buckets × ~7 panes each) | Follow-on to the IA hybrid ship. Each bucket sweep: chrome swap (`<PaneHeader>` / `<EmptyState>` / `<SaveBar>`) + per-loader URL move + redirect-shim entry + kbar action map row. ~40 panes total. Parallelizable — agents per-bucket don't conflict on file paths. Spec follow-up table at `docs/roadmap/admin/admin-information-architecture.md` § "Follow-up: per-area sweep". |
