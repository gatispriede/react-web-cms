...
| C9 | [production-caching.md](production-caching.md) | L | **Shipped (2026-05-02)** — `bootId` + per-feature version stamps in Redis (`cacheVersion.ts`), `X-Cms-Cache-Tag` header on public responses, Caddy SWR config fragment + [runbook](../runbooks/caddy-cache.md), DataLoader fold-in via `ServiceLoader.batchAccessors` (Posts is the demo) |
