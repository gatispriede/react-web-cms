...
| Item | Track | Size | Notes |
|------|-------|------|-------|
| ~~[admin-information-architecture.md](admin/admin-information-architecture.md)~~ | ~~admin~~ | ~~L-XL~~ | **SHIPPED 2026-05-16 (hybrid)** — new 6-bucket taxonomy (Site / Content / Commerce / People / Analytics / System) wired into the top bar + area rails + AdminView union; full pane audit at `docs/roadmap/_meta/admin-pane-inventory.md`; 4 demonstrator panes' URLs moved with 301 shim entries; shared `<PaneHeader>` / `<EmptyState>` / `<SaveBar>` + 4 rhythm tokens adopted in 6 demonstrator panes (one per bucket). Remaining ~40 panes' chrome swap + their per-loader URL moves captured as per-area sweep follow-ups in the spec doc's "Follow-up: per-area sweep" section. See [shipped.md](shipped.md#2026-05-16--admin-information-architecture-new-taxonomy--shared-pane-components). |
