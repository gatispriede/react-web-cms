...
| ~~[invoicing-and-bookkeeping.md](storefront/invoicing-and-bookkeeping.md)~~ | ~~7~~ | ~~L~~ | **SHIPPED 2026-05-16** — `IInvoice` + `ICreditNote` schemas, gap-free `InvoiceSequence` numbering, `InvoiceService` (issue / void / CSV export), deterministic `@react-pdf/renderer` PDF generation, `OrderService.finalize` hook with rollback on invoice failure, MCP coverage (`invoice.list/get/export`, `creditNote.create/list/get`), admin `commerce/invoices` pane, customer-side download link on `/account/orders/[id]`. See [shipped.md](shipped.md#2026-05-16--invoicing-and-bookkeeping-eu-compliant-invoices-credit-notes-pdf-csv-export). |
...
| ~~`next-i18next/pages` → `/client` sweep~~ | ~~2~~ | ~~S~~ | **SHIPPED 2026-05-16** — every `ui/` consumer migrated onto `next-i18next/client` (`useT`) + dead `ui/client/lib/adminSsr.ts` Pages-Router helper deleted. Unblocks dropping the pages-router peer-dep and the eventual `next-auth@5` jump. See [shipped.md](shipped.md#2026-05-16--next-i18nextpages--client-sweep). |
...
| Item | Track | Size | Notes |
|------|-------|------|-------|
| ~~[admin-information-architecture.md](admin/admin-information-architecture.md)~~ | ~~admin~~ | ~~L-XL~~ | **SHIPPED 2026-05-16 (hybrid)** — new 6-bucket taxonomy (Site / Content / Commerce / People / Analytics / System) wired into the top bar + area rails + AdminView union; full pane audit at `docs/roadmap/_meta/admin-pane-inventory.md`; 6 demonstrator panes' URLs moved with 301 shim entries; shared `<PaneHeader>` / `<EmptyState>` / `<SaveBar>` + 4 rhythm tokens adopted in 6 demonstrator panes (one per bucket). **Re-pivoted same day to 5-bucket task-driven taxonomy** (Build / Content / Settings / Analytics / System) — operator feedback showed noun buckets read fine but operators kept bouncing between plausible-looking homes; refined taxonomy organises by *what am I doing in this area*. Settings absorbs every configuration surface hierarchically. Three demonstrator URLs re-pointed (Footer / Users / Invoices); 301 shim covers both pre-IA and first-ship 6-bucket URLs. Remaining ~40 panes' chrome swap + their per-loader URL moves captured as per-area sweep follow-ups below (restructured around the 5 buckets). See [shipped.md](shipped.md#2026-05-16--admin-information-architecture-re-pivot--5-bucket-task-driven-taxonomy). |
| admin-ia per-area sweeps (Content / Settings / Analytics / System) | admin | M-L per bucket (4 buckets covering ~40 panes total; Settings is the largest with sub-tree fan-out into chrome / theme / features/* / access) | Follow-on to the IA hybrid + re-pivot ship. Each bucket sweep: chrome swap (`<PaneHeader>` / `<EmptyState>` / `<SaveBar>`) + per-loader URL move + redirect-shim entry + kbar action map row. Parallelizable — agents per-bucket don't conflict on file paths. Settings/features/* sweeps double as feature toggle/config consolidation (enable + config on one page per feature). Spec follow-up table at `docs/roadmap/admin/admin-information-architecture.md` § "Follow-up: per-area sweep" + URL map at `docs/roadmap/_meta/admin-pane-inventory.md`. |
