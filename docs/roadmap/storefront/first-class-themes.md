...
| ~~`agency`~~ | ~~Design / dev / marketing agencies~~ | ~~Bold, motion-forward~~ | ~~Hide-on-down / show-on-up~~ | ~~Brand-led XXL~~ | **SHIPPED 2026-05-16** — visual pass across module catalogue: full-bleed photo Hero cover, asymmetric magazine ProjectGrid with named editorial cards, single-card ProjectCard, magazine-spread Gallery masonry variant, large pull-quote Testimonials + TestimonialWall, oversized accent-glyph Manifesto, hairline-credit KeyValueDossier, case-card List variant, agency SectionHeading vocabulary, multi-column brand-led XXL Footer. See [shipped.md](../shipped.md#2026-05-16--first-class-themes-agency-visual-pass). |
...
| ~~`saas-landing`~~ | ~~Software products~~ | ~~Sharp, animated, gradient accents, **dark-default**~~ | ~~Shrink-on-scroll~~ | ~~Minimal~~ | **SHIPPED 2026-05-16** — visual pass across module catalogue: violet-gradient hero halo, gradient-bordered pricing tier, mono changelog version pills, monochrome→colour LogoCloud hover, dark-card BlogFeed, vs-competitor ComparisonTable with gradient outline, MetricsCallout/StatsStrip/StatsCard big numbers, faceted-filter chips, manifesto pull-quote. See [shipped.md](../shipped.md#2026-05-16--first-class-themes-saas-landing-visual-pass). |
